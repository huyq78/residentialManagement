...
export default function UpdateAbsence() {
  ...
  const absence = location.state.state;

  useEffect(() => {
    let curr = new Date(absence.date.from);
    var from = curr.toISOString().substring(0, 10);
    curr = new Date(absence.date.to);
  ...
  const handleSubmit = (id) => {
    fetch(`http://localhost:5000/absence/update/${id}`, {
      method: "PATCH",
      headers: {
          ...
          <form>
            ...
            <div className="flex flex-wrap">
              <button
                ...
                type="button"
                onClick={()=>handleSubmit(absence._id)}
              >
                Tạo
              </button>
            </div>
