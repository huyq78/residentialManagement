...
export default function UpdateStay() {
  ...
  const stay = location.state.state;

  useEffect(() => {
    let curr = new Date(stay.date.from);
    var from = curr.toISOString().substring(0, 10);
    curr = new Date(stay.date.to);
  ...
  const handleSubmit = (id) => {
    fetch(`http://localhost:5000/stay/update/${id}`, {
      method: "PATCH",
      headers: {
